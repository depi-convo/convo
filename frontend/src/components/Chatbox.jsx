    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-800 transition-colors duration-300 overflow-hidden ">
      ...
      <div className="flex items-center p-3 border-b border-gray-200 dark:border-slate-700">
        ...
        <div className="ml-3 min-w-0">
          <Label className="font-medium text-gray-900 dark:text-white truncate">{user.name}</Label>
          <Label className="text-xs text-green-500">{user.isOnline ? "Online" : "Offline"}</Label>
        </div>
      </div>
      ...
      >
        {messages.map((message, index) => (
          <div key={index} className={cn("flex flex-col", message.sender === "user" ? "items-end" : "items-start")}>
            <div
              className={cn(
            ...
            >
              ...
              <div className="text-xs text-gray-400 mt-1 flex items-center justify-end">
                ...
                {message.sender === "user" && <span className="ml-2">{message.read ? "read" : "delivered"}</span>}
              </div>
            </div>
      ...
      {/* Input */}
      <div className="p-4 border-t flex space-x-2 bg-white dark:bg-slate-800 mt-auto">
        <Input
          value={input}
